...
public class SmsUtility implements SMSHandler {
	...
	public String handleSmsRequest(String smsContent, String senderDeviceId) {
		...
		String response = "";
		try {
			// Authenticated user for this session with public access inside
			// application(working as DAO)
			...
			if (VirtualDB.loggedInUser != null) {
				...
				case 1:
					if ("BALANCE".equalsIgnoreCase(smsContents[0])) {
						// Provides static reference to current logged in user
						response = userManager.getBalance(VirtualDB.loggedInUser.getName()).toString();
					} else {
						response = "ERR : UNKNOWN COMMAND";
					}
					break;
				case 3:
					if (userManager.existsUser(smsContents[2])) {
						if ("SEND".equalsIgnoreCase(smsContents[0])) {
							...
							transferManager.sendMoney(VirtualDB.loggedInUser.getName(), smsContents[2], amt);

						} else if ("TOTAL".equalsIgnoreCase(smsContents[0])
								&& "SENT".equalsIgnoreCase(smsContents[1])) {
							List<BigDecimal> allTransactions = transferManager
									.getAllTransactions(VirtualDB.loggedInUser.getName(), smsContents[2]);
				...
				case 4: // Check if user exists in the DB
					if (userManager.existsUser(smsContents[2]) && userManager.existsUser(smsContents[3])) {
						if ("TOTAL".equalsIgnoreCase(smsContents[0]) && "SENT".equalsIgnoreCase(smsContents[1])) {
							...
							allTransactions = transferManager.getAllTransactions(VirtualDB.loggedInUser.getName(),
									smsContents[3]);
							if (allTransactions != null) {
								for (BigDecimal bd : allTransactions) {
